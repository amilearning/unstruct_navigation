...
from unstruct_navigation.models.feature_encoder import DynPredModel


class mppi_offroad:
    def __init__(self, Config = None, pred_model = DynPredModel):
        ...
        self.mppi = MPPI(pred_model,dynamics, costs, sampling, self.MPPI_config)
        self.mppi.reset()
    ...
    def obtain_states(self, odom, imu , grid_center):
        odom = deepcopy(odom)
        imu = deepcopy(imu)
        quaternion = (
        ...
        init_del_x = odom.pose.pose.position.x - grid_center[0]
        init_del_y = odom.pose.pose.position.y - grid_center[1]        
        init_del_xy = torch.tensor([init_del_x, init_del_y]).cuda()
        self.pred_model.set_init_delxy(init_del_xy)        
        
        state_for_pred = torch.tensor([ local_vel[0],local_vel[1],local_vel[2], local_ang_vel[0],local_ang_vel[1],local_ang_vel[2],rpy[0],rpy[1],rpy[2]]).cuda()
        self.pred_model.set_vehicle_state(state_for_pred)
        
        return state_for_pred, init_del_xy
