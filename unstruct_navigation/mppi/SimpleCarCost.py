...
class SimpleCarCost(torch.nn.Module):
    ...
    def forward(self, state, controls):
        ...
        obj_dist_thres = 0.5
        obj_pose = torch.tensor([3.8,-0.3]).cuda()
        obj_pose = obj_pose.unsqueeze(0).repeat(state.shape[1],1).unsqueeze(0)
        
        obj_vec = obj_pose - state[:,:,-1,:2]
        obj_dist = torch.linalg.norm(obj_vec, dim=-1)
        obj_cost = torch.clip(torch.tanh(-1*(torch.abs(obj_dist - obj_dist_thres))+1), 0.0, 1.0)
        ...
        return (running_cost.mean(dim=0)).sum(dim=1) + cost_to_go.mean(dim=0)  + obj_cost.squeeze()