...
def imgmsg_to_cv2(ros_img, desired_encoding='passthrough'):
    assert ros_img.encoding == 'bgra8'

    dtype = 'uint8'
    n_channels  = 4        
    dtype = np.dtype(dtype)
    dtype = dtype.newbyteorder('>' if ros_img.is_bigendian else '<')

    img_buf = np.asarray(ros_img.data, dtype=dtype) if isinstance(ros_img.data, list) else ros_img.data

    if n_channels == 1:
        im = np.ndarray(shape=(ros_img.height, int(ros_img.step/dtype.itemsize)),
                        dtype=dtype, buffer=img_buf)
        im = np.ascontiguousarray(im[:ros_img.height, :ros_img.width])
    else:
        im = np.ndarray(shape=(ros_img.height, int(ros_img.step/dtype.itemsize/n_channels), n_channels),
                        dtype=dtype, buffer=img_buf)
        im = np.ascontiguousarray(im[:ros_img.height, :ros_img.width, :])

    # If the byte order is different between the message and the system.
    if ros_img.is_bigendian == (sys.byteorder == 'little'):
...
def ros_image_to_torch(ros_img, desired_encoding="rgb8", device="cpu"):
    if type(ros_img).__name__ == "_sensor_msgs__Image" or isinstance(ros_img, Image):
        np_image = imgmsg_to_cv2(ros_img, desired_encoding=desired_encoding)
...
def numpy_to_ros_image(np_img, desired_encoding="rgb8"):
    ...
    Returns:
        _type_: _description_
    """
    ros_image = cv2_to_imgmsg(np_img, encoding=desired_encoding)
    return ros_image
